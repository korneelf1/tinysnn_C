...
def create_connection_from_template(name, state_dict, state_name, bias_name=None, mask1=None, mask2=None):
    if mask1 is None and mask2 is None:
        weights = state_dict[state_name]
        bias = state_dict[bias_name] if bias_name is not None else None
    ...
    if bias_name is not None:
        create_connection_from_template_with_weights_and_bias(name, weights, bias)
    else:
...
def create_connection_from_template_with_weights(name, weights):
    ...
    for i in range(weights.size()[0]):
        for j in range(weights.size()[1]):
            weights_string += f"{weights[i, j].item():4f}f, "
...
def create_connection_from_template_with_weights_and_bias(name, weights, bias):
    input_size = weights.size()[1]
    output_size = weights.size()[0]
    weights_string = '{'
    for i in range(weights.size()[0]):
    ...
    for i in range(bias.size()[0]):
        bias_string += f"{bias[i].item():4f}f, "
    bias_string = bias_string[:-2] + '}'
    ...
    template = 'param/templates/test_connection_with_bias_file.templ'
    out = f'param/controller/test_controller_{name}_file.h'
...
def create_snntorch_neuron_from_template(name, state_dict, state_name, sigmoid=False, mask=None):
    ...
    for i in range(hidden_size):
        if mask is not None:
            if mask[i] == 0:
                continue
        leak_i = torch.tensor(0.0)
        leak_v = state_dict[f"{state_name}.beta"][i]
        if sigmoid:
            leak_i = torch.sigmoid(leak_i)
            leak_v = torch.sigmoid(leak_v)
        else:
